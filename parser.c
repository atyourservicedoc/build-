...
void lenv_put(lenv* e, lval* k, lval* v) {
    ...
    /*to see if variable actually exists*/
    for (int i = 0; i < e->count; i++) {
...
void lval_del(lval* v) {
    ...
    switch (v->type) {
        /*do nothing specail for number type*/
        case LVAL_NUM: break;
        case LVAL_FUN:
            if (!v->builtin) {
                lenv_del(v->env);
        ...
        case LVAL_SEXPR:
            ...
            /*and free memory for the pointers*/
            free(v->cell);
            break;
    ...
    /*finally free the memory for the struct itself*/
    free(v);
}
...
/*print a lval followed by newline*/
...
lval* lval_eval(lenv* e, lval* v) {
    ...
    /*all other lvals return the same way*/
    return v;
}
...
lval* builtin_op(lenv* e, lval* a, char* op) {
    ...
    /*while ther are still elements remaining*/
    while (a->count > 0) {
...
lval* builtin_load(lenv* e, lval* a) {
    ...
    if(mpc_parse_contents(a->cell[0]->str, Lispy, &r)) {
        ...
        /*create new errror message using it*/
...
int lval_eq(lval* x, lval* y) {
    ...
    switch(x->type) {
        ...
        /*if list compare each individual element*/
        case LVAL_QEXPR:
        case LVAL_SEXPR:
            if (x->count != y->count) { return 0;}
            for (int i = 0; i < x->count; i++) {
                /*if any element not equal, whole list is not equal*/
...
int main(int argc, char** argv) {
    ...
    string  : /\"(\\\\.|[^\"])*\"/ ;        \
    comment : /;[^\\r\\n]*/ ;                    \
  ", Number, Symbol, Sexpr, Qexpr, Expr, Lispy, String, Comment);

    //setup environemnt and builtins
    lenv* e = lenv_new();
    lenv_add_builtins(e);
